    ...
    <span>
      {parts.map((part, i) =>
        part.toLowerCase() === highlight.toLowerCase() ? (
          <strong
            key={i}
            className="text-black font-extrabold underline decoration-2 decoration-gray-300 underline-offset-2"
          >
            {part}
          </strong>
        ) : (
...
const Searchbar: React.FC<SearchbarProps> = ({ containerClassName }) => {
  ...
  // ADDED: Logic to clear all recent searches
  ...
  return (
    <>
      ...
      <div
        ...
        ref={containerRef}
      >
        {/* Closed Search Input Trigger */}
        {!isOpen && (
          <div
            className="w-full bg-white border border-gray-300 hover:border-black hover:shadow-lg 
          ...
          >
            <FaSearch className="text-gray-400 group-hover:text-black mr-3 transition-colors duration-300" />
            <span className="text-gray-500 group-hover:text-black transition-colors duration-300 select-none flex-1 font-medium">
              Find your destination...
            </span>
            <div className="bg-gray-50 rounded px-2 py-1 text-xs text-gray-400 font-bold border border-gray-200 group-hover:border-gray-400 group-hover:text-gray-600 transition-all">
              CMD+K
            </div>
          </div>
        )}
        ...
        {isOpen && (
          <div className="absolute top-0 left-0 w-full transform transition-all duration-200">
            <div className="bg-white rounded-xl shadow-2xl overflow-hidden border border-gray-200 ring-1 ring-black/5">
              {/* Header / Input Area */}
              <div className="relative border-b border-gray-100 bg-white">
                <FaSearch className="absolute left-5 top-1/2 -translate-y-1/2 text-black text-lg" />
                <input
                  ref={inputRef}
                  type="text"
                  value={query}
                  ...
                  className="w-full pl-14 pr-10 py-5 text-lg text-black placeholder-gray-400 focus:outline-none bg-transparent font-medium"
                />
                {query && (
                  <button
                    onClick={() => {
                  ...
                  <>
                    {recent.length > 0 && (
                      <div className="mb-2">
                        {/* 
                            MODIFIED: 
                            Changed header to flex container to include "Clear All" button.
                            Kept original styles for the heading text.
                        */}
                        ...
                        {recent.map((item, index) => (
                          <div
                            ...
                            className={`px-5 py-3 flex items-center justify-between cursor-pointer group transition-all duration-200
                                ${
                                  selectedIndex === index
                                    ? "bg-zinc-100 border-l-4 border-black pl-4"
                          ...
                          >
                            ...
                            <button
                              ...
                              className="text-gray-300 hover:text-black opacity-0 group-hover:opacity-100 transition-all p-1"
                            >
                              <FaTimes size={12} />
                            </button>
                          </div>
              ...
              {/* Footer */}
              <div className="bg-zinc-50 px-5 py-3 border-t border-gray-200 flex justify-between items-center text-[10px] md:text-xs text-gray-500 uppercase tracking-wide">
                <span className="font-semibold">Mount Treks Search</span>
                <div className="flex items-center gap-3 sm:flex">
                  <span className="flex items-center gap-1">
                    <kbd className="font-sans bg-white border border-gray-300 rounded px-1.5 py-0.5 shadow-sm text-gray-700 font-bold">
                      ↑
                    </kbd>
                    <kbd className="font-sans bg-white border border-gray-300 rounded px-1.5 py-0.5 shadow-sm text-gray-700 font-bold">
                      ↓
